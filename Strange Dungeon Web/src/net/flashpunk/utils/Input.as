package net.flashpunk.utils
{
	...
	public class Input
	{
		...
		public static function update():void
		{
			...
			for (var i:uint = 0; i < touch.length; i++)
			{
				if (touch[i].touchPressed) touch[i].touchPressed = false;
				if (touch[i].touchReleased) touch[i].touchReleased = false;
			}
		...
		/**
		 * Multitouch inputs (up to 5 at once)
		 */
		public static var touch:Array = new Array(new Touch(), new Touch(), new Touch(), new Touch(), new Touch());
		
		/** @private Event handler for touch press. */
		private static function onTouchBegin(e:TouchEvent):void
		{
			touch[e.touchPointID].update(e.stageX / FP.screen.scale, e.stageY / FP.screen.scale);
			touch[e.touchPointID].touchDown = true;
			touch[e.touchPointID].touchUp = false;
			touch[e.touchPointID].touchPressed = true;
		}
		
		/** @private Event handler for touch release. */
		private static function onTouchEnd(e:TouchEvent):void
		{
			touch[e.touchPointID].update(e.stageX / FP.screen.scale, e.stageY / FP.screen.scale);
			touch[e.touchPointID].touchDown = false;
			touch[e.touchPointID].touchUp = true;
			touch[e.touchPointID].touchTapped = false;
			touch[e.touchPointID].touchReleased = true;
		}
		
		/** @private Event handler for touch move events */
